...
// Main Component
export const UserListCRUD: React.FC = () => {
  ...
  }, [metadata.hasPrevPage, page]);

  const handleNextPage = useCallback(() => {
    if (metadata.hasNextPage) {
      setPage(page + 1);
    }
  }, [metadata.hasNextPage, page]);
  ...
  const handleUserModalSave = (userData: Omit<User, 'id'>) => {
    if (selectedUser) {
  ...
  const handleUserModalCancel = useCallback(() => {
    setIsUserModalOpen(false);
    setSelectedUser(null);
  }, []);
